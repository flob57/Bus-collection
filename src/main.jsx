...
// Structure prête pour les données Notion. Les vraies données remplaceront ce jeu local
// lorsque le proxy sécurisé Notion sera branché sur Cloudflare.
const demoVehicles = [
  ...
  return (
    <>
      <header className="hero">
        ...
        <p>Ta collection personnelle d’autobus, autocars et tramways, réseau par réseau.</p>
      </header>
      <section className="networks">
        <div className="section-title">Choisir un réseau</div>
        <div className="network-grid">
          {networks.map((network) => (
            <button className="network-card" key={network.id} onClick={() => onSelect(network.id)} style={{ '--accent': network.accent }}>
              <NetworkLogo network={network} />
              <div><h2>{network.name}</h2><span>{network.city}</span><p>{network.description}</p></div>
...
function VehicleCard({ vehicle, accent }) {
  ...
  return (
    <article className={`vehicle-card ${retired ? 'retired' : ''}`} style={{ '--accent': accent }}>
      <div className="vehicle-photo">
        {vehicle.image ? <img src={vehicle.image} alt={`${vehicle.brand} ${vehicle.model} ${vehicle.park}`} /> : <div className="no-photo"><span>📷</span><small>Photo à ajouter</small></div>}
        {retired && <b className="retired-badge">RÉFORMÉ</b>}
      </div>
      <div className="vehicle-body">
        <div className="vehicle-title"><strong>#{vehicle.park}</strong><span>{vehicle.type}</span></div>
        <h3>{vehicle.brand} {vehicle.model}</h3>
        <div className="vehicle-meta">
          <div><small>IMMATRICULATION</small><b>{vehicle.registration || '—'}</b></div>
          <div><small>MISE EN CIRCULATION</small><b>{vehicle.date || '—'}</b></div>
        </div>
      </div>
    </article>
  );
...
function Collection({ networkId, onBack }) {
  ...
  const vehicles = demoVehicles.filter((v) => v.network === networkId);
  ...
  const filtered = useMemo(() => vehicles.filter((v) => {
    ...
    const matchesType = type === 'Tous' || v.type === type;
    const retired = /réform|retir|hors/i.test(v.status || '');
    return matchesSearch && matchesType && (showRetired || !retired);
  }).sort((a, b) => (b.date || '').localeCompare(a.date || '')), [vehicles, query, type, showRetired]);

  return (
    <>
      <header className="collection-head" style={{ '--accent': network.accent }}>
        <button className="back" onClick={onBack}>← Accueil</button>
        <div className="collection-brand"><NetworkLogo network={network} /><div><div className="eyebrow">COLLECTION</div><h1>{network.name}</h1><p>{network.city}</p></div></div>
      </header>
      <section className="toolbar">
        <div className="search"><span>⌕</span><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Parc, immatriculation, marque, modèle…" /></div>
        <div className="filters">
          <select value={type} onChange={(e) => setType(e.target.value)}>{types.map((t) => <option key={t}>{t}</option>)}</select>
          <button className={showRetired ? 'toggle active' : 'toggle'} onClick={() => setShowRetired(!showRetired)}>Réformés</button>
        </div>
      </section>
      ...
      <section className="vehicle-grid">
        {filtered.map((vehicle) => <VehicleCard key={vehicle.id} vehicle={vehicle} accent={network.accent} />)}
      </section>
      {filtered.length === 0 && <div className="empty">Aucun véhicule ne correspond à ta recherche.</div>}
    </>
  );
}
