(() => {
  ...
  function render(vehicles) {
    ...
    const old = document.querySelector('.weekly-new'); if (old) old.remove();
  ...
  async function load() {
    try {
      ...
      if (render(Array.isArray(data.vehicles) ? data.vehicles : [])) {
        if (timer) { clearInterval(timer); timer = null; }
      }
    } catch (error) {
      console.warn('Bus Collection: impossible de charger les nouveaux véhicules', error);
    }
  }
  function start() {
    load();
    if (!timer) timer = setInterval(() => { if (document.querySelector('.networks')) load(); }, 1000);
